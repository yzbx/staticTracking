...
void doubleThresholdConnect(const Mat &highThresholdMask,const Mat &lowThresholdMask,Mat &outputMergedMask){
    Mat outputMask=Mat::zeros(highThresholdMask.rows+2,highThresholdMask.cols+2,CV_8UC1);
    ...
    for(int i=0;i<img_rows;i++){
        for(int j=0;j<img_cols;j++){
            Point p(j,i);
            //BUG high threshold donot ensure low threshold
            if(highThresholdMask.at<uchar>(i,j)>0&&lowThresholdMask.at<uchar>(i,j)>0&&outputMask.at<uchar>(i+1,j+1)==0){
                count++;
                Mat mask=Mat::zeros(img_rows+2,img_cols+2,CV_8UC1);
                floodFill(lowThresholdMask,mask,p,Scalar(255),&rect,Scalar(0),Scalar(0),8|FLOODFILL_FIXED_RANGE|FLOODFILL_MASK_ONLY);
                CV_Assert(mask.at<uchar>(i+1,j+1)!=0);
                CV_Assert(lowThresholdMask.at<uchar>(i,j)==255);
                outputMask|=mask;
                //                imshow("lowThresholdMask",lowThresholdMask);
...
//    imshow("highMask",highThresholdMask);
//    imshow("lowMask",lowThresholdMask);
//    imshow("outputMask",outputMask*100);

    outputMask=outputMask>0;
    outputMergedMask.release();
    outputMergedMask.create(img_rows,img_cols,CV_8UC1);
    for(int i=0;i<img_rows;i++){
        for(int j=0;j<img_cols;j++){
            outputMergedMask.at<uchar>(i,j)=outputMask.at<uchar>(i+1,j+1);
        }
    }
//    imshow("mergedMask",outputMergedMask);
//    waitKey(0);
//    qDebug()<<"count="<<count;
}
